    ...
    try:
        while 1:
            ...
            if weather_info != "天气参数获取失败!":
                dr.rectangle([0, 16, 127, 63], fill=0)
                dr.text([0, 16], "{:^5}".format(weather_info.split(",")[2]), font=get_font(23)[1], fill=1)
                dr.text([0, 52], "{:^10}".format(weather_info.split(",")[4][4:]), font=get_font(10)[1], fill=1)
                dr.text([30, 52], "{:^5}".format(weather_info.split(",")[5][2:]), font=get_font(10)[1], fill=1)
